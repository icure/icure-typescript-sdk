...
export class IccClassificationXApi extends IccClassificationApi implements EncryptedEntityXApi<models.Classification> {
  ...
  /**
   * Shares the classifications with the provided ids with one or more delegates, using the same share options for all of
   * them.
   *
   * Unlike {@link shareWith} this method does not need the decrypted classifications, does not return them, and does not
   * fail because of a single classification or delegate: the outcome of each (classification, delegate) pair is reported in
   * the returned {@link ShareByIdResult}. Ids of classifications that don't exist or that the current user can't read are
   * reported in {@link ShareByIdResult.notFoundIds} and are otherwise ignored.
   * @param ids the ids of the classifications to share. Duplicates are ignored.
   * @param delegates associates the id of the data owners which will be granted access to the classifications to the
   * following sharing options:
   * - shareSecretIds specifies which secret ids of each of the classifications should be shared: with
   ...
   * - shareEncryptionKey specifies if the encryption key of the classifications should be shared: this is needed for the
   * delegate to be able to decrypt their content. Defaults to {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * - sharePatientId specifies if the id of the patient the classifications refer to should be shared with the delegate. Defaults to
   * {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * @return a promise which will be completed with the outcome of the operation for each (classification, delegate) pair.
   */
  async shareById(
    ids: string[],
    delegates: {
    ...
    return this.crypto.xapi.shareById(
      ...
      EntityWithDelegationTypeName.Classification,
      (x) => this.findClassificationsDelegationsStubsByIds(x),
      (x) => this.bulkShareClassificationsMinimal(x)
    )
  }
