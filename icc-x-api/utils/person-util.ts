...
export function garnishPersonWithName<P extends models.Patient | models.HealthcareParty>(person: P, nameUse: models.PersonName.UseEnum, lastName?: string, firstName?: string, fullName: string = `${lastName} ${firstName ?? ""}`) : P {
  return {
    ...person,
    names: (person.names ?? []).concat([new models.PersonName({
      lastName: lastName,
