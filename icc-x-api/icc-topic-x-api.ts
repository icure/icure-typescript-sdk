...
export class IccTopicXApi extends IccTopicApi implements EncryptedEntityXApi<models.Topic> {
    ...
    async newInstance(
        ...
        const extraDelegations = {
            ...Object.fromEntries(
                (user.autoDelegations?.all ?? []).map((d) => [d, AccessLevelEnum.WRITE])
            ),
        ...
        const sfk = patient ? options.preferredSfk ?? (await this.crypto.confidential.getAnySecretIdSharedWithParents({entity: patient, type: 'Patient'})) : undefined
    ...
    decrypt(topics: Array<models.Topic>) {
        return Promise.all(topics.map((topic) => this.crypto.xapi.decryptEntity(topic, 'Topic', (x) => new models.Topic(x)).then(({entity}) => entity)))
    }

    encrypt(topics: Array<models.Topic>): Promise<Array<models.Topic>> {
        return Promise.all(
            topics.map((p) => this.crypto.xapi.tryEncryptEntity(p, 'Topic', this.encryptedKeys, true, false, (x) => new models.Topic(x)))
        )
    }
    ...
    async addParticipantWithTopic(body: { dataOwnerId: string; topicRole: TopicRole }, topic: Topic): Promise<Topic> {
        const updatedTopic = await this.shareWith(
            body.dataOwnerId,
            topic,
            {
