...
export class IccDocumentXApi extends IccDocumentApi implements EncryptedEntityXApi<models.Document> {
  ...
  /**
   * Shares the documents with the provided ids with one or more delegates, using the same share options for all of
   * them.
   *
   * Unlike {@link shareWith} this method does not need the decrypted documents, does not return them, and does not
   * fail because of a single document or delegate: the outcome of each (document, delegate) pair is reported in
   * the returned {@link ShareByIdResult}. Ids of documents that don't exist or that the current user can't read are
   * reported in {@link ShareByIdResult.notFoundIds} and are otherwise ignored.
   * @param ids the ids of the documents to share. Duplicates are ignored.
   * @param delegates associates the id of the data owners which will be granted access to the documents to the
   * following sharing options:
   * - shareSecretIds specifies which secret ids of each of the documents should be shared: with
   ...
   * - shareEncryptionKey specifies if the encryption key of the documents should be shared: this is needed for the
   * delegate to be able to decrypt their content. Defaults to {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * - shareMessageId specifies if the id of the message the documents refer to (if any) should be shared with the delegate. Defaults to
   * {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * @return a promise which will be completed with the outcome of the operation for each (document, delegate) pair.
   */
  async shareById(
    ids: string[],
    delegates: {
    ...
    return this.crypto.xapi.shareById(
      ...
      EntityWithDelegationTypeName.Document,
      (x) => this.findDocumentsDelegationsStubsByIds(x),
      (x) => super.bulkShareDocumentMinimal(x)
    )
  }
   ...
   */
  getConflictsForEntityWithUser(user: models.User, entityId: string): Promise<Array<models.Document>> {
    return super
  ...
  async declareConflictWinnerWithUser(
    user: models.User,
    request: models.ConflictResolutionRequest<models.Document>
  ): Promise<models.ConflictResolutionResult<models.Document>> {
   ...
   * @return the decrypted conflicting revisions of the document.
   */
  getConflictsForEntityInGroupWithUser(user: models.User, groupId: string, entityId: string): Promise<Array<models.Document>> {
    return super
