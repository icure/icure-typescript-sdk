...
export class IccHelementXApi extends IccHelementApi implements EncryptedEntityXApi<models.HealthElement> {
  ...
  /**
   * Shares the health elements with the provided ids with one or more delegates, using the same share options for all of
   * them.
   *
   * Unlike {@link shareWith} this method does not need the decrypted health elements, does not return them, and does not
   * fail because of a single health element or delegate: the outcome of each (health element, delegate) pair is reported in
   * the returned {@link ShareByIdResult}. Ids of health elements that don't exist or that the current user can't read are
   * reported in {@link ShareByIdResult.notFoundIds} and are otherwise ignored.
   * @param ids the ids of the health elements to share. Duplicates are ignored.
   * @param delegates associates the id of the data owners which will be granted access to the health elements to the
   * following sharing options:
   * - shareSecretIds specifies which secret ids of each of the health elements should be shared: with
   ...
   * - shareEncryptionKey specifies if the encryption key of the health elements should be shared: this is needed for the
   * delegate to be able to decrypt their content. Defaults to {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * - sharePatientId specifies if the id of the patient the health elements refer to should be shared with the delegate. Defaults to
   * {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * @return a promise which will be completed with the outcome of the operation for each (health element, delegate) pair.
   */
  async shareById(
    ids: string[],
    delegates: {
    ...
    return this.crypto.xapi.shareById(
      ...
      EntityWithDelegationTypeName.HealthElement,
      (x) => this.listHealthElementsDelegationsStubById(x),
      (x) => this.bulkShareHealthElementsMinimal(x)
    )
  }
