...
import { DelegationMembersDetails } from './SecureDelegationsSecurityMetadataDecryptor'
...
export class SecurityMetadataDecryptor {
  ...
  async decryptSecureDelegationsUsingKnownExchangeData(
    ...
    const retrievedByExchangeDataId = await this.exchangeData.getDecryptionDataKeyByIds([...toSearchById], true)
    const res: { [entityId: string]: string[] } = {}
    for (const entity of entities) {
      const currEntityRes: Set<string> = new Set([])
  ...
  async decryptSecureDelegationsUsingExchangeDataMap(
    ...
    if (toSearchByExchangeDataMap.size == 0) return {}
    const exchangeDataMaps = await this.exchangeDataMap.getExchangeDataMapBatch([...toSearchByExchangeDataMap])
    ...
    const retrievedByExchangeDataId = await this.exchangeData.getDecryptionDataKeyByIds(
      [...new Set(Object.values(exchangeDataIdByDelegationKey))],
      true
    )
    const res: { [entityId: string]: string[] } = {}
    for (const entity of entities) {
  ...
  async decryptAll(
    ...
    const allExtractedWithDataOwners: { decrypted: string; dataOwnersWithAccess: string[] }[] = await this.extractFromLegacyDelegations(
      entity,
