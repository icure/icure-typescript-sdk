...
export class IccReceiptXApi extends IccReceiptApi implements EncryptedEntityXApi<models.Receipt> {
  ...
  /**
   * Shares the receipts with the provided ids with one or more delegates, using the same share options for all of
   * them.
   *
   * Unlike {@link shareWith} this method does not need the decrypted receipts, does not return them, and does not
   * fail because of a single receipt or delegate: the outcome of each (receipt, delegate) pair is reported in
   * the returned {@link ShareByIdResult}. Ids of receipts that don't exist or that the current user can't read are
   * reported in {@link ShareByIdResult.notFoundIds} and are otherwise ignored.
   * @param ids the ids of the receipts to share. Duplicates are ignored.
   * @param delegates associates the id of the data owners which will be granted access to the receipts to the
   * following sharing options:
   * - shareSecretIds specifies which secret ids of each of the receipts should be shared: with
   ...
   * - shareEncryptionKey specifies if the encryption key of the receipts should be shared: this is needed for the
   * delegate to be able to decrypt their content. Defaults to {@link ShareMetadataBehaviour.IF_AVAILABLE}.
   * @return a promise which will be completed with the outcome of the operation for each (receipt, delegate) pair.
   */
  async shareById(
    ids: string[],
    delegates: {
    ...
    return this.crypto.xapi.shareById(
      ...
      EntityWithDelegationTypeName.Receipt,
      (x) => this.findReceiptsDelegationsStubsByIds(x),
      (x) => this.bulkShareReceiptMinimal(x)
    )
  }
