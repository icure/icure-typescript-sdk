...
describe('Document api', () => {
  ...
  it('should allow to create and retrieve main attachments', async () => {
    ...
    const retrievedData = await documentApi!.getDocumentAttachment(document.id!, 'ignored')
  ...
  it('should allow to update main attachment', async () => {
    ...
    const updated = await documentApi!.setDocumentAttachment(document.id!, undefined, data1, sampleUti)
    expect(updated.mainUti).to.equal(sampleUti[0])
    assert(arrayEquals(updated.otherUtis!, sampleUti.slice(1)))
    assert(bufferEquals(await documentApi!.getDocumentAttachment(document.id!, 'ignored'), data1))
    const updated2 = await documentApi!.setDocumentAttachment(document.id!, undefined, data2)
    expect(updated2.mainUti).to.equal(sampleUti[0])
    assert(arrayEquals(updated2.otherUtis!, sampleUti.slice(1)))
    assert(bufferEquals(await documentApi!.getDocumentAttachment(document.id!, 'ignored'), data2))
  })
  ...
  it('should allow to update utis in main attachment', async () => {
    ...
    const updated2 = await documentApi!.setDocumentAttachment(document.id!, undefined, data, sampleUti)
    expect(updated2.mainUti).to.equal(sampleUti[0])
    assert(arrayEquals(updated2.otherUtis!, sampleUti.slice(1)))
  })
