...
describe('CSM-93, CSM-138', async function () {
  ...
  it(
    'A patient should be able to create a new key pair and related maintenance tasks (even if crypto cache is populated),' +
      ' then retrieve existing data after give access back',
    async function () {
      ...
      // CSM-138: Populate crypto cache for patApisLost
      await patApisLost.patientApi.getPatientWithUser(patUser, patUser.patientId!)
      // CSM-138: add new key pair should work even if the cache is populated
      ...
      const dataOwnerConcernedPubKey = (foundTasks[0] as MaintenanceTask).properties?.find((prop) => prop.id == 'dataOwnerConcernedPubKey')
