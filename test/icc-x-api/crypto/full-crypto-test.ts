import { Apis, WebCryptoPrimitives, IcureApi, ShaVersion, ua2hex } from '../../../icc-x-api'
    ...
    const storage = await testStorageWithKeys([
      {
        ...
        pairs: keyStrings.map((key) => {
          return { keyPair: key, shaVersion: ShaVersion.Sha256 }
        }),
      },
    ])
...
async function createPartialsForHcp(
  ...
  const key = await primitives.RSA.generateKeyPair(ShaVersion.Sha256)
