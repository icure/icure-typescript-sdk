import { b64Url2ua, RSAUtilsImpl, ShaVersion, ua2ab } from '../../../icc-x-api'
...
describe('ArrayBuffer methods', () => {
  ...
  describe('convertKeysFormat', () => {
    it('should manage jwk conversions for private keys gracefully', async () => {
      ...
      const jwk3 = spkiToJwk(hex2ua(pubKey), ShaVersion.Sha1)
      const spki = jwk2spki(jwk3)
      const jwk4 = spkiToJwk(hex2ua(spki), ShaVersion.Sha1)
    ...
    it('should convert spki to jwk in a coherent way', async () => {
      ...
      const pubKey = await healthcarePartyApi.getCurrentHealthcareParty().then((hcp) => hcp.publicKey!)
      const jwk1 = spkiToJwk(hex2ua(pubKey), ShaVersion.Sha1)

      const rsaKey1 = await rsa.importKey('jwk', jwk1, ['encrypt'], ShaVersion.Sha1)
      const rsaKey2 = await rsa.importKey('spki', ua2ab(hex2ua(pubKey)), ['encrypt'], ShaVersion.Sha1)
      const jwk2 = await rsa.exportKey(rsaKey2, 'jwk')
      const rsaKey3 = await rsa.importKey('jwk', jwk2, ['encrypt'], ShaVersion.Sha1)
