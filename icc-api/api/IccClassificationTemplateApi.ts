...
export class IccClassificationTemplateApi {
 ...
 createClassificationTemplate(body?: ClassificationTemplate): Promise<ClassificationTemplate> {
    ...
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              new ClassificationTemplate(doc.body as JSON)
 ...
 findClassificationTemplatesByHCPartyPatientForeignKeys(hcPartyId: string, secretFKeys: string): Promise<Array<ClassificationTemplate>> {
    ...
    const _url = this.host + `/classificationTemplate/byHcPartySecretForeignKeys` + "?ts=" + new Date().getTime()  + (hcPartyId ? "&hcPartyId=" + encodeURIComponent(String(hcPartyId)) : "") + (secretFKeys ? "&secretFKeys=" + encodeURIComponent(String(secretFKeys)) : "")
    let headers = this.headers
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
          
              (doc.body as Array<JSON>).map(it => new ClassificationTemplate(it))
      )
      .catch(err => this.handleError(err))
}
 ...
 listClassificationTemplates(startKey?: string, startDocumentId?: string, limit?: number): Promise<PaginatedListClassificationTemplate> {
    ...
    const _url = this.host + `/classificationTemplate` + "?ts=" + new Date().getTime()  + (startKey ? "&startKey=" + encodeURIComponent(String(startKey)) : "") + (startDocumentId ? "&startDocumentId=" + encodeURIComponent(String(startDocumentId)) : "") + (limit ? "&limit=" + encodeURIComponent(String(limit)) : "")
    let headers = this.headers
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
          
              new PaginatedListClassificationTemplate(doc.body as JSON)
 ...
 newClassificationTemplateDelegations(classificationTemplateId: string, body?: Array<Delegation>): Promise<ClassificationTemplate> {
    ...
    const _url = this.host + `/classificationTemplate/${encodeURIComponent(String(classificationTemplateId))}/delegate` + "?ts=" + new Date().getTime() 
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
