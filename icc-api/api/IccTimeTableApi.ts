...
export class IccTimeTableApi {
 ...
 createTimeTable(body?: TimeTable): Promise<TimeTable> {
    ...
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              new TimeTable(doc.body as JSON)
 ...
 getTimeTablesByAgendaId(agendaId: string): Promise<Array<TimeTable>> {
    ...
    const _url = this.host + `/timeTable/byAgendaId` + "?ts=" + new Date().getTime()  + (agendaId ? "&agendaId=" + encodeURIComponent(String(agendaId)) : "")
    let headers = this.headers
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
          
              (doc.body as Array<JSON>).map(it => new TimeTable(it))
      )
      .catch(err => this.handleError(err))
}
 ...
 getTimeTablesByPeriodAndAgendaId(startDate: number, endDate: number, agendaId: string): Promise<Array<TimeTable>> {
    ...
    const _url = this.host + `/timeTable/byPeriodAndAgendaId` + "?ts=" + new Date().getTime()  + (startDate ? "&startDate=" + encodeURIComponent(String(startDate)) : "") + (endDate ? "&endDate=" + encodeURIComponent(String(endDate)) : "") + (agendaId ? "&agendaId=" + encodeURIComponent(String(agendaId)) : "")
    let headers = this.headers
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
