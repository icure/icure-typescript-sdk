...
export class IccGroupApi {
 ...
 createGroup(id: string, name: string, password: string, server?: string, q?: number, n?: number, body?: DatabaseInitialisation): Promise<Group> {
    ...
    const _url = this.host + `/group/${encodeURIComponent(String(id))}` + "?ts=" + new Date().getTime()  + (name ? "&name=" + encodeURIComponent(String(name)) : "") + (server ? "&server=" + encodeURIComponent(String(server)) : "") + (q ? "&q=" + encodeURIComponent(String(q)) : "") + (n ? "&n=" + encodeURIComponent(String(n)) : "")
 ...
 getReplicationInfo1(id: string): Promise<ReplicationInfo> {
    ...
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              new ReplicationInfo(doc.body as JSON)
 ...
 initDesignDocs(id: string, clazz?: string, warmup?: boolean): Promise<Unit> {
    ...
    const _url = this.host + `/group/${encodeURIComponent(String(id))}/dd` + "?ts=" + new Date().getTime()  + (clazz ? "&clazz=" + encodeURIComponent(String(clazz)) : "") + (warmup ? "&warmup=" + encodeURIComponent(String(warmup)) : "")
    let headers = this.headers
 ...
 listGroups(): Promise<Array<Group>> {
    ...
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              (doc.body as Array<JSON>).map(it => new Group(it))
      )
      .catch(err => this.handleError(err))
}
 ...
 registerNewGroupAdministrator(body?: RegistrationInformation): Promise<RegistrationSuccess> {
    ...
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              new RegistrationSuccess(doc.body as JSON)
 ...
 resetStorage(id: string, q?: number, n?: number, body?: ListOfIds): Promise<Unit> {
    ...
    const _url = this.host + `/group/${encodeURIComponent(String(id))}/reset/storage` + "?ts=" + new Date().getTime()  + (q ? "&q=" + encodeURIComponent(String(q)) : "") + (n ? "&n=" + encodeURIComponent(String(n)) : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
 ...
 solveConflicts(id: string, limit?: number, warmup?: boolean): Promise<Array<IdWithRev>> {
    ...
    const _url = this.host + `/group/${encodeURIComponent(String(id))}/conflicts` + "?ts=" + new Date().getTime()  + (limit ? "&limit=" + encodeURIComponent(String(limit)) : "") + (warmup ? "&warmup=" + encodeURIComponent(String(warmup)) : "")
    let headers = this.headers
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => 
