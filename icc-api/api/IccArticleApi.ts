...
export class IccArticleApi {
   ...
   *
   * @param articleIds an array containing the ids of the articles to delete.
   * @return a Promise that will resolve in an array of DocIdentifiers of the successfully delete articles.
   */
  deleteArticles(articleIds: string[]): Promise<Array<DocIdentifier>> {
    const response =
      getApiVersionFromUrl(this.host) == ApiVersion.V1
        ? XHR.sendCommand(
            'DELETE',
            this.host + `/article/${encodeURIComponent(articleIds.join(','))}` + '?ts=' + new Date().getTime(),
            this.headers,
            null,
            this.fetchImpl,
            undefined,
