...
export class IccAnonymousAccessApi {
  ...
  handleError(e: XHR.XHRError): never {
    throw e
      ...
      * @param limit 
      */
 getAvailabilitiesByPeriodAndCalendarItemTypeId(groupId: string, userId: string, getCalendarItemTypeId: string, startDate: number, endDate: number, hcpId: string, limit?: number): Promise<Array<number>> {
    let _body = null
    
    const _url = this.host + `/aa/available/inGroup/${encodeURIComponent(String(groupId))}/forUser/${encodeURIComponent(String(userId))}/type/${encodeURIComponent(String(getCalendarItemTypeId))}` + "?ts=" + new Date().getTime()  + (startDate ? "&startDate=" + encodeURIComponent(String(startDate)) : "") + (endDate ? "&endDate=" + encodeURIComponent(String(endDate)) : "") + (hcpId ? "&hcpId=" + encodeURIComponent(String(hcpId)) : "") + (limit ? "&limit=" + encodeURIComponent(String(limit)) : "")
 ...
 listAppointmentTypesForUser(groupId: string, userId: string, startDate: number, endDate: number): Promise<Array<CalendarItemType>> {
    ...
    const _url = this.host + `/aa/appointmentType/inGroup/${encodeURIComponent(String(groupId))}/forUser/${encodeURIComponent(String(userId))}` + "?ts=" + new Date().getTime()  + (startDate ? "&startDate=" + encodeURIComponent(String(startDate)) : "") + (endDate ? "&endDate=" + encodeURIComponent(String(endDate)) : "")
    let headers = this.headers
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
          
              (doc.body as Array<JSON>).map(it => new CalendarItemType(it))
      )
      .catch(err => this.handleError(err))
}
