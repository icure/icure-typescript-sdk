...
export class IccAgendaApi {
 ...
 getAgendas(): Promise<Array<Agenda>> {
    ...
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
              ...
              (doc.body as Array<JSON>).map(it => new Agenda(it))
      )
      .catch(err => this.handleError(err))
}
 ...
 getReadableAgendasForUser(userId: string): Promise<Array<Agenda>> {
    ...
    const _url = this.host + `/agenda/readableForUser` + "?ts=" + new Date().getTime()  + (userId ? "&userId=" + encodeURIComponent(String(userId)) : "")
    let headers = this.headers
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => 
