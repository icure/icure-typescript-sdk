...
export class IccCalendarItemApi {
  ...
  async getConflictsForEntity(entityId: string): Promise<Array<CalendarItem>> {
    const _url = this.host + `/calendarItem/conflicts/of?entityId=${encodeURIComponent(String(entityId))}` + '&ts=' + new Date().getTime()
    let headers = await this.headers
    return XHR.sendCommand('GET', _url, headers, null, this.fetchImpl, undefined, this.authenticationProvider.getAuthService())
      .then((doc) => (doc.body as Array<JSON>).map((it) => new CalendarItem(it)))
      .catch((err) => this.handleError(err))
  ...
  /**
   * Automatically resolves the conflicting revisions of the calendar items with the given ids, according to the provided
   * {@link ConflictResolutionStrategy}.
   * @summary Automatically solve the conflicts of the calendar items with the given ids.
   * @param entityIds the ids of the calendar items to solve the conflicts for.
  ...
  async autoSolveConflicts(
    ...
    const _url = this.host + `/calendarItem/conflicts/solve?strategy=${encodeURIComponent(String(strategy))}` + '&ts=' + new Date().getTime()
    let headers = await this.headers
    headers = headers.filter((h) => h.header !== 'Content-Type').concat(new XHR.Header('Content-Type', 'application/json'))
   ...
   * @summary Automatically solve the conflicts of the calendar items with the given ids, in the given group.
  ...
  async autoSolveConflictsInGroup(
    ...
    const _url =
      this.host +
      `/calendarItem/inGroup/${encodeURIComponent(String(groupId))}/conflicts/solve?strategy=${encodeURIComponent(String(strategy))}` +
      '&ts=' +
      new Date().getTime()
    let headers = await this.headers
    headers = headers.filter((h) => h.header !== 'Content-Type').concat(new XHR.Header('Content-Type', 'application/json'))
